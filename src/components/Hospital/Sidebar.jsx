...
const Sidebar = () => {
  ...
  return (
    <div className="bg-[#1C3B6F] text-white h-screen w-64 flex flex-col fixed left-0 top-0 transition-all duration-500 ease-in-out">
      {/* Header */}
      <div className="flex items-center justify-between h-16 px-4 ">
        <div className="flex items-center space-x-2">
          <div className="w-8 h-8 bg-teal-500 rounded-lg flex items-center justify-center">
            <span className="text-white font-bold font-poppins">M</span>
          </div>
      ...
      <nav className="mt-8 px-4 flex-1 overflow-y-auto">
        <ul className="space-y-2">
          {menuItems.map((item) => (
            <li key={item.id}>
              <button
                className={`w-full flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-300 ease-in-out font-gothambook ${
                  isActive(item.path)
              ...
              {item.submenu && (
                <ul
                  className={`ml-6 mt-2 space-y-2 overflow-hidden transition-all duration-500 ease-in-out font-gothambook ${
                ...
                >
                  {item.submenu.map((subItem) => (
                    <li key={subItem.id}>
                      <button
                        className={`w-full flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 font-gothambook ${
                      ...
                      >
                        <span
                          className={`w-2 h-2 rounded-full mr-3 font-gothambook ${
      ...
      <div className="px-[40px] py-[22px]  font-gothambook">
          <OutlineButton
            onClick={() => router.push("/")}
            color="red"
            size="medium"
