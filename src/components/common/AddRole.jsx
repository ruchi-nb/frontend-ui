...
const DoctorForm = ({ onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Call the API
      const result = await addDoctorToHospital(payload);
  ...
  return (
    <form className="space-y-8" onSubmit={handleSubmit}>
      ...
      {/* Credentials Section - Now using the reusable component */}
      <Credentials
        username={username}
        password={form.password}
        genMode={form.genMode}
        loading={loading}
        onPasswordChange={(value) => setForm(f => ({ ...f, password: value }))}
        onGenModeChange={(value) => setForm(f => ({ ...f, genMode: value }))}
